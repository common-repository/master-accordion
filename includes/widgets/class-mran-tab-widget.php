 ...
 */
class MRAN_Tab_Widgets extends WP_Widget {
	...
	public function __construct() {
		$widget_ops  = array(
			'classname'                   => 'mran_tab_widget',
			'description'                 => esc_html__( 'Widget for Tab', 'master-accordion' ),
			'customize_selective_refresh' => true,
		);
		$control_ops = array( 'width' => 350, 'height' => 350 );
		parent::__construct( 'mran_tab_widget', esc_html__( 'Tab Post Widget', 'master-accordion' ), $widget_ops, $control_ops );
	}
	...
	public function widget( $args, $instance ) {
		...
		if ( $query->have_posts() ):
			?>
			<div class="mran-tab-template mran-widget mran-tab-<?php echo $templates; ?>">
				<div class="mran-tab <?php echo esc_attr($style); ?>">
					<ul class="mran-tab-list" style="background:<?php echo sanitize_hex_color($title_wraper_bg); ?>;">
						<?php 
						$current_tab = 0; 
							...
							<li class="mran-tab-item-wrap">
								<div class="mran-tab-title <?php echo esc_attr($tab_class); ?>" style="background:<?php echo sanitize_hex_color($title_background); ?>; color:<?php echo sanitize_hex_color($title_color); ?>;">
									<?php if(!empty($tab_icon)): ?>
										<i
										class="mran-tab-icon fa <?php echo esc_attr($tab_icon); ?>" 
										style="color:<?php echo sanitize_hex_color($title_color); ?>;"
										></i> <?php 
								endif; 
									?><a class="mran-post-link" href="#post_tab_<?php echo get_the_ID(); ?>" style="color:inherit;"><?php the_title(); ?></a>
									
								</div>
							</li>
						<?php endwhile; ?>
					</ul>
					<div class="mran-tab-content-wraper" style="background: <?php echo sanitize_hex_color($content_wraper_bg); ?>;">
						<?php 
						$current_tab = 0;
						while ( $query->have_posts() ):$query->the_post(); 
							$current_tab++;
							$tab_class = ($current_tab==$active_item) ? ' current ' : '';
							?>
							<div class="mran-tab-content <?php echo esc_attr($tab_class); ?>" id="post_tab_<?php echo get_the_ID(); ?>" style="background:<?php echo sanitize_hex_color($content_background); ?>; color:<?php echo sanitize_hex_color($content_color); ?>;">
	...
	public function form( $instance ) {
		...
		$list_all_tabs = array(
			'general'	=>	array(
				'id'	=> 'mran_tab_widget_general'.esc_attr($this->number),
				'label'	=> esc_html__('General', 'master-accordion'),
			),
			'layout'	=>	array(
				'id'	=> 'mran_tab_widget_layout'.esc_attr($this->number),
				'label'	=> esc_html__('Layout', 'master-accordion'),
			),
			'design'	=>	array(
				'id'	=> 'mran_tab_widget_design'.esc_attr($this->number),
		...
		<div class="mran-tab-wraper">
			...
			<div class="mran-tab-content-wraper">
				<div id="<?php echo esc_attr($list_all_tabs['general']['id']); ?>" class="mran-tab-content <?php echo ($active_tab_type=='general') ? 'mran-content-active' : ''; ?>" >
					<p>
						<label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php esc_html_e( 'Title:', 'master-accordion' ); ?></label>
						<input class="widefat" id="<?php echo $this->get_field_id( 'title' ); ?>"
							   name="<?php echo $this->get_field_name( 'title' ); ?>" type="text"
							   value="<?php echo esc_attr( $title ); ?>"/>
					</p>
					<p>
						<label for="<?php echo $this->get_field_id( 'post_type' ); ?>"><?php esc_html_e( 'Post Type:', 'master-accordion' ); ?></label>
						<?php
						$args           = array(
							'public' => true,
						);
						...
						?>
						<select class="widefat mran-widget-post-type" data-accordion-value="post_type"
					...
					<p><label
							for="<?php echo $this->get_field_id( 'taxonomy' ); ?>"><?php esc_html_e( 'Taxonomy:', 'master-accordion' ); ?></label>
						<?php
						$all_object_taxonomies = get_object_taxonomies( $post_type, 'objects' );
						?>
						<select class="widefat mran-widget-taxonomy" data-accordion-value="taxonomy"
					...
					<p><label for="<?php echo $this->get_field_id( 'term' ); ?>"><?php esc_html_e( 'Term:', 'master-accordion' ); ?></label>
						<?php
					...
					<p><label
							for="<?php echo $this->get_field_id( 'no_of_post' ); ?>"><?php esc_html_e( 'Show no of post:', 'master-accordion' ); ?></label>
					...
					?>
					<p><label for="<?php echo $this->get_field_id( 'content_type' ); ?>"><?php esc_html_e( 'Content Type:', 'master-accordion' ); ?></label>
				...
				<div id="<?php echo esc_attr($list_all_tabs['layout']['id']); ?>" class="mran-tab-content <?php echo ($active_tab_type=='layout') ? 'mran-content-active' : ''; ?> " >
					<p>
						<label for="<?php echo $this->get_field_id( 'active_item' ); ?>"><?php esc_html_e( 'Active Tab Item:', 'master-accordion' ); ?></label>
						...
						<span><?php esc_html_e('Please set active item on load(zero for collapse all)', 'master-accordion'); ?></span>
					</p>
					<p>
						<label for="<?php echo $this->get_field_id( 'templates' ); ?>"><?php esc_html_e( 'Template:', 'master-accordion' ); ?></label>
						<?php $all_templates = mran_accordion_templates(); ?>
					...
					<p>
						<label for="<?php echo $this->get_field_id( 'style' ); ?>"><?php esc_html_e( 'Style:', 'master-accordion' ); ?></label>
						<?php
				...
				<div id="<?php echo esc_attr($list_all_tabs['design']['id']); ?>" class="mran-tab-content <?php echo ($active_tab_type=='design') ? 'mran-content-active' : ''; ?> " >
					<div class="mran_widget_field">
						<label for="<?php echo $this->get_field_id( 'tab_icon' ); ?>"><?php esc_html_e( 'Tab Icon:', 'master-accordion' ); ?></label>
						<div class="mran_widget_icon_wrap">
							<input 
								class="widefat mran_icon_picker" 
								id="<?php echo $this->get_field_id( 'tab_icon' ); ?>"
							   	name="<?php echo $this->get_field_name( 'tab_icon' ); ?>" 
							   	type="text"
							   value="<?php echo esc_attr( $tab_icon ); ?>"/>
							   <label class="mran_icon fa <?php echo esc_attr( $tab_icon ); ?>" for="<?php echo $this->get_field_id( 'tab_icon' ); ?>"></label>
						</div>
					</div>
					<div class="mran_widget_field">
					...
					<div class="mran_widget_field">
						<label for="<?php echo $this->get_field_id( 'title_wraper_bg' ); ?>"><?php esc_html_e( 'Title Wraper Background:', 'master-accordion' ); ?></label>
						<input class="mran_color_picker" id="<?php echo $this->get_field_id( 'title_wraper_bg' ); ?>"
							   name="<?php echo $this->get_field_name( 'title_wraper_bg' ); ?>" type="text"
							   value="<?php echo esc_attr( $title_wraper_bg ); ?>"/>
					</div>
					<div class="mran_widget_field">
						<label for="<?php echo $this->get_field_id( 'content_color' ); ?>"><?php esc_html_e( 'Content Color:', 'master-accordion' ); ?></label>
						<input class="mran_color_picker" id="<?php echo $this->get_field_id( 'content_color' ); ?>"
							   name="<?php echo $this->get_field_name( 'content_color' ); ?>" type="text"
							   value="<?php echo esc_attr( $content_color ); ?>"/>
					</div>
					<div class="mran_widget_field">
						<label for="<?php echo $this->get_field_id( 'content_background' ); ?>"><?php esc_html_e( 'Content Background:', 'master-accordion' ); ?></label>
						<input class="mran_color_picker" id="<?php echo $this->get_field_id( 'content_background' ); ?>"
							   name="<?php echo $this->get_field_name( 'content_background' ); ?>" type="text"
							   value="<?php echo esc_attr( $content_background ); ?>"/>
					</div>
					<div class="mran_widget_field">
						<label for="<?php echo $this->get_field_id( 'content_wraper_bg' ); ?>"><?php esc_html_e( 'Content Wraper Background:', 'master-accordion' ); ?></label>
						<input class="mran_color_picker" id="<?php echo $this->get_field_id( 'content_wraper_bg' ); ?>"
...
register_widget( 'MRAN_Tab_Widgets' );