 ...
 */
class MRAN_Accordion_Widgets extends WP_Widget {
	...
	public function __construct() {
		$widget_ops  = array(
			'classname'                   => 'mran_accordion_widget',
			'description'                 => esc_html__( 'Widget for Accordion', 'master-accordion' ),
			'customize_selective_refresh' => true,
		);
		$control_ops = array( 'width' => 350, 'height' => 350 );
		parent::__construct( 'mran_accordion_widget', esc_html__( 'Accordion Post Widget', 'master-accordion' ), $widget_ops, $control_ops );
	}
	...
	public function widget( $args, $instance ) {
		...
		if ( $query->have_posts() ):
			?>
			<div class="mran-accordion-template mran-widget mran-<?php echo $templates; ?>">
				<div class="mran-accordion <?php echo esc_attr($style); ?>">
					<ul class="mran-accordion-list">
						...
						while ( $query->have_posts() ):$query->the_post(); 
							...
							<li class="mran-accordion-item-wrap">
								<div class="mran-accordion-title <?php echo esc_attr($active_class); ?>" style="background:<?php echo sanitize_hex_color($title_background); ?>; color:<?php echo sanitize_hex_color($title_color); ?>;">
									<span data-href="#mran_<?php echo $mran_post_slug.get_the_ID(); ?>"><?php the_title(); ?></span>
									<?php if(!empty($dropdown_icon)): ?>
										<i 
										class="mran-toggle-icon fa <?php echo esc_attr($active_icon); ?>" 
								...
								<div class="mran-content <?php echo esc_attr($active_class); ?>" id="mran_<?php echo $mran_post_slug.get_the_ID(); ?>" style="background:<?php echo sanitize_hex_color($content_background); ?>; color:<?php echo sanitize_hex_color($content_color); ?>; <?php echo esc_attr($active_style); ?>">
	...
	public function form( $instance ) {
		...
		$list_all_tabs = array(
			'general'	=>	array(
				'id'	=> 'mran_accordion_widget_general'.esc_attr($this->number),
				'label'	=> esc_html__('General', 'master-accordion'),
			),
			'layout'	=>	array(
				'id'	=> 'mran_accordion_widget_layout'.esc_attr($this->number),
				'label'	=> esc_html__('Layout', 'master-accordion'),
			),
			'design'	=>	array(
				'id'	=> 'mran_accordion_widget_design'.esc_attr($this->number),
		...
		<div class="mran-tab-wraper">
			...
			<div class="mran-tab-content-wraper">
				...
				<div id="<?php echo esc_attr($list_all_tabs['layout']['id']); ?>" class="mran-tab-content <?php echo ($active_tab_type=='layout') ? 'mran-content-active' : ''; ?> " >
					<p>
						<label for="<?php echo $this->get_field_id( 'active_item' ); ?>"><?php esc_html_e( 'Active Accordion Item:', 'master-accordion' ); ?></label>
...
register_widget( 'MRAN_Accordion_Widgets' );