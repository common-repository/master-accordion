 ...
 */
class MRAN_Term_Tab_Widgets extends WP_Widget {
	...
	public function __construct() {
		$widget_ops  = array(
			'classname'                   => 'mran_term_tab_widget',
			'description'                 => esc_html__( 'Widget for Term Tab', 'master-accordion' ),
			'customize_selective_refresh' => true,
		);
		$control_ops = array( 'width' => 350, 'height' => 350 );
		parent::__construct( 'mran_term_tab_widget', esc_html__( 'Tab Term Widget', 'master-accordion' ), $widget_ops, $control_ops );
	}
	...
	public function widget( $args, $instance ) {
		...
		if ( $all_terms ):
			...
			<div class="mran-tab-template mran-widget mran-tab-<?php echo $templates; ?>">
				<div class="mran-tab <?php echo $style; ?>">
					<ul class="mran-tab-list">
						...
						foreach ( $all_terms as $key => $term_detail ):
							...
							<li class="mran-tab-item-wrap">
								<div class="mran-tab-title <?php echo esc_attr($active_class); ?>" style="background:<?php echo sanitize_hex_color($title_background); ?>; color:<?php echo sanitize_hex_color($title_color); ?>;">
									<?php if(!empty($tab_icon)): ?>
										...
										></i>
									<?php endif; ?>
									<a class="mran-post-link" href="#mran_<?php echo $term_detail->slug.$term_detail->term_id; ?>"><?php echo $term_detail->name; ?></a>
								</div>
							</li>
						<?php endforeach; ?>
					</ul>
					<div class="mran-tab-content-wraper">
						...
						$current_item++; 
							...
							?>
							<div class="mran-tab-content <?php echo esc_attr($active_class); ?>" id="mran_<?php echo $term_detail->slug.$term_detail->term_id; ?>" style="background:<?php echo sanitize_hex_color($content_background); ?>; color:<?php echo sanitize_hex_color($content_color); ?>;">
	...
	public function form( $instance ) {
		...
		$list_all_tabs = array(
			'general'	=>	array(
				'id'	=> 'mran_term_accordion_general'.esc_attr($this->number),
				'label'	=> esc_html__('General', 'master-accordion'),
			),
			'layout'	=>	array(
				'id'	=> 'mran_term_accordion_layout'.esc_attr($this->number),
				'label'	=> esc_html__('Layout', 'master-accordion'),
			),
			'design'	=>	array(
				'id'	=> 'mran_term_accordion_design'.esc_attr($this->number),
		...
		<div class="mran-tab-wraper">
			...
			<div class="mran-tab-content-wraper">
				<div id="<?php echo esc_attr($list_all_tabs['general']['id']); ?>" class="mran-tab-content <?php echo ($active_tab_type=='general') ? 'mran-content-active' : ''; ?>">
					...
					<p><label
							for="<?php echo $this->get_field_id( 'no_of_term' ); ?>"><?php esc_html_e( 'Show no of term:', 'master-accordion' ); ?></label>
				...
				<div id="<?php echo esc_attr($list_all_tabs['design']['id']); ?>" class="mran-tab-content <?php echo ($active_tab_type=='design') ? 'mran-content-active' : ''; ?> " >
					<div class="mran_widget_field">
						...
						<div class="mran_widget_icon_wrap">
							<input class="widefat mran_icon_picker" id="<?php echo $this->get_field_id( 'tab_icon' ); ?>"
...
register_widget( 'MRAN_Term_Tab_Widgets' );